import { countEnabledPlugins, getEnabledPluginNames, loadPlugins } from '../../../pluginLoader.js'
...
import { testCORS } from '../../core/helper/testCors.js';
...
const initialize = () => {
  ...
  Alpine.store('geonodeCustomLayerFilter', {
    ...
    visibleAttributes: function() {
      return this.nameSelectOptions
          .filter(option => option.visible && !this.isBlacklistedKey(option.attribute))
    ...
    updateallFormValues: function() {
     ...
     rows.forEach(row => {
      ...
      if (hasNameValue && operator && value) {
        const cqlValue = String(value).replace(/'/g, "''");
        concatenatedValues += `("${name}"${operator}'${cqlValue}')${joinStr}`;
      }
    });
