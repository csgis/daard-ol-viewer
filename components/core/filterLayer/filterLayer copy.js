...
const createButtonMarkup = () => {
  ...
  <button 
    ...
    :class="$store.filterLayer.componentIsActive ? 'bg-danger' : 'btn-light'"
  ...
  rightMiddleSlot.insertAdjacentHTML('beforeend', filterFormButtonHtml);
}
...
const initialize = () => {
  ...
  document.getElementById('filterOffcanvas').addEventListener('hide.bs.offcanvas', function () {
    ...
    filterLayerStore.componentIsActive = false;
    console.log(filterLayerStore.componentIsActive);
  });

  Alpine.store('pluginStatus').increasePluginLoadingStatus();

  console.log("hello from filter from")
